...
<script>
	...
	function addBox(result) {
		$("#addClassify").empty();
		$("#addClassify").append('<option value = "">请选择</option>');
	...
	function toRemove(ids) {
		...
		if (ids) {
			...
			$app.confirm("删除数据不可恢复，确定要删除吗？", function() {
				$.post(
						'${path}/memMemberAdd/deleteMemberAddList.do?ids='
								+ ids, function(data) {
    ...
    <div class="rightinfo">
		<div class="explain_col">
    		<form id="searchForm"  method="post">
    			<span>分类:<select name="addClassify" id = "addClassify">
    						</select>
    			</span>
    			<div style="display: inline;" class="lxr_multipleSelect" data-name="deptid" data-model="deptSelect"> </div>
    				...
    				<span>关键词：</span>
    				<input name="tradeName" placeholder="关键词"  class="form-control input-sm w200" type="text" style="display: inline;" >
    	...
    	<table id="manTable" class="table_list" data-toggle="table"
			data-url="${path}/memMemberAdd/memberAddList.do" data-pagination="true"
			...
			<thead>
				<tr>
					...
					<th data-field="memberName">昵称</th>
					<th data-field="memberPhone">手机号</th>
					<th data-field="tradeName">商品名</th>
					<th data-field="addTime" data-formatter="dateFormatter">收藏时间</th>
				</tr>
			</thead>
		</table>
