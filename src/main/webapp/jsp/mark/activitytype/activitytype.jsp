...
<script>
	//添加
	function toAdd(){
		$app.dialog('${path}/marketing/activity/addactivitytypeView.do',function(){
    		refTable();
		});
	}
	...
	function toRemove(ids) {
		...
		if (ids) {
			$app.confirm("删除数据不可恢复，确定要删除吗？", function() {
				$.post('${path}/marketing/activity/deactivitytype.do?ids=' + ids,
						function(data) {
							if (data.code == 1) {
	...
	//编辑
	function toEdit(id) {
		$app.dialog('${path}/marketing/activity/updateactivitytypeView.do?id=' + id,
				function() {
					refTable();
				});
	}
    ...
    <div class="rightinfo">
		<div class="explain_col">
    		<form id="searchForm" name="searchForm"  method="post">
    			<label>分类名称：</label><input type="text" name="userName" class="form-control input-sm w260" style="display: inline;">&nbsp;
    	...
    	<table id="manTable" class="table_list" data-toggle="table"
			data-url="${path}/marketing/activity/activitytypes.do" data-pagination="true"
			...
			<thead>
				<tr>
					...
					<th data-field="typename">分类名称</th>
					<th data-field="typedesc">简要说明</th>
