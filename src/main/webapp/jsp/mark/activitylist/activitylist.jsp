...
<script>
	//添加
	function toAdd(){
		$app.dialog('${path}/marketing/activity/addactivitylistView.do',function(){
    		refTable();
		});
	}
	...
	function toRemove(ids) {
		...
		if (ids) {
			$app.confirm("删除数据不可恢复，确定要删除吗？", function() {
				$.post('${path}/marketing/activity/deactivitylist.do?ids=' + ids,
						function(data) {
							if (data.code == 1) {
	...
	//编辑
	function toEdit(id) {
		$app.dialog('${path}/marketing/activity/updateactivitylistView.do?id=' + id,
				function() {
					refTable();
				});
	}
    ...
    function stateFormatter(value,row,index){
    	if(value=='1'){
    		return '<span class="label label-success label-lg">正在活动</span>';
    	}else if(value=='2'){
    		return '<span class="label label-danger arrowed">活动结束</span>';
    	}else{
    		return "";
    	}
    }
    ...
    <div class="rightinfo">
		<div class="explain_col">
    		<form id="searchForm" name="searchForm"  method="post">
    			<label>活动名称：</label><input type="text" name="activityname" class="form-control input-sm w260" style="display: inline;">&nbsp;
    			<label>状态</label>
    			<select name="state">
    	...
    	<table id="manTable" class="table_list" data-toggle="table"
			data-url="${path}/marketing/activity/activitylists.do" data-pagination="true"
			...
			<thead>
				<tr>
					...
					<th data-field="activitytype">活动分类</th>
					<th data-field="activityname">活动名称</th>
					<th data-field="state" data-formatter="statusFormatter">状态</th>
					<th data-field="state" data-formatter="stateFormatter">活动状态</th>
