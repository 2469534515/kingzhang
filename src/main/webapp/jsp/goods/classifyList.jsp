...
<script>
	//添加
	function toAdd(){
		$app.dialog('${path}/classify/toAddClassify.do',function(){
    		refTable();
		});
	}
	//删除
	function toRemove(ids){
		...
		if(ids){
			...
			$app.confirm("删除数据不可恢复，确定要删除吗？",function(){
				 $.post('${path}/classify/deleteCalssify.do?ids='+ids,function(data){
    ...
    function toEdit(id){
    		...
    		$app.dialog('${path}/classify/toUpdateClassify.do?id='+id,function(){
        		refTable();
    		});
...
<!-- 		</div> -->
    	...
    	<table id="manTable" class="table_list" data-toggle="table"
			data-url="${path}/classify/getClassifyList.do" data-pagination="true"
			...
			<thead>
				<tr>
					...
					<th data-field="goodsClassifyName">商品分类名称</th>
					<th data-field="goodsClassifyPhoto" data-formatter="imageFormatter">商品分类图片</th>
					<th data-field="goodsClassifySort">排序</th>
